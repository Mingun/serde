...
use crate::de::{self, Deserializer, EnumAccess, IntoDeserializer, MapAccess, SeqAccess, Visitor};
use crate::lib::*;
...
primitive_deserializer!(
    /// A deserializer holding an `u128`.
    ///
...
impl<'de, T> IntoDeserializer<'de, T::Error> for SomeDeserializer<T>
where
    T: Deserializer<'de>,
{
    type Deserializer = Self;
...
impl<T> NewtypeDeserializer<T> {
    #[allow(missing_docs)]
    pub fn new(deserializer: T) -> Self {
        NewtypeDeserializer { deserializer }
    }
}
...
impl<'de, T> IntoDeserializer<'de, T::Error> for NewtypeDeserializer<T>
where
    T: Deserializer<'de>,
{
    type Deserializer = Self;
