        ...
        V: Visitor<'de>,
    {
        match self.peek_token() {
            Token::Enum { name: n } if name == n => {
                ...
                visitor.visit_enum(DeserializerEnumVisitor { de: self, optional_token: false })
            }
            ...
            | Token::ByteBuf(_)
            | Token::U8(_) | Token::U16(_) | Token::U32(_) | Token::U64(_) => {
                visitor.visit_enum(DeserializerEnumVisitor { de: self, optional_token: true })
            }
            _ => self.deserialize_any(visitor),
        }
    }
